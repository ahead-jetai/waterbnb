...
export default function HomePage() {
  return (
    <main>
      ...
      <section id="explore" aria-labelledby="featured-heading" className="container-p mt-12">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 id="featured-heading" className="text-2xl font-semibold text-muted">Featured Listings</h2>
            <p className="text-slate-600">Handpicked stays on boats and houseboats worldwide.</p>
          </div>
          <a className="btn btn-secondary" href="#">View all</a>
        </div>
        <ul role="list" className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {listings.map((l) => (
            <li key={l.id}>
              <ListingCard item={l} />
            </li>
